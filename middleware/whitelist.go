...
func WhiteList(ctx iris.Context)  {
	...
	if !ok {
		golog.Errorf("读取配置 security.whitelist 异常")
		// 这里先放行，免得配置异常造成不可访问
		ctx.Next()
		return
	}
	...
	golog.Debugf("X-Forwarded-For: [%s]",forwardedIP)
	//clientHost := strings.Split(ctx.Host(),":")[0]
	//golog.Debugf("remoteAddr: %s",ctx.RemoteAddr())
	//golog.Debugf("remoteAddr: %s",ctx.Request().RemoteAddr)
	clientHost := strings.Split(ctx.RemoteAddr(),":")[0]
	//golog.Debugf("clientHost: %s",clientHost)
	// 本机直接放行
	...
	if !safeIp{
		golog.Warnf("非授权IP %s %s 试图访问 %s", forwardedIP ,clientHost, ctx.RequestPath(false))
